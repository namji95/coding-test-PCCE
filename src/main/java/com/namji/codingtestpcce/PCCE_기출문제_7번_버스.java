...
public class PCCE_기출문제_7번_버스 {
  public static void main(String[] args) {
        /*
        문제 설명
        정수 seat와 2차원 문자열 배열 passengers가 매개변수로 주어집니다.
        ...
        기점에서 출발한 버스가 순서대로 방문한 정거장에서 승객이 승/하차한 정보를 담은 passengers가 주어질 때,
    ...
    answer = solution(seat, passengers);
    System.out.println(answer);
  }
  public static int solution(int seat, String[][] passengers) {
    int num_passenger = 0;
    for(int i=0; i<passengers.length; i++){
      num_passenger += func4(passengers[i]); // func빈칸;
      num_passenger -= func3(passengers[i]); // func빈칸;
    }
    int answer = func1(seat - num_passenger); // func빈칸;
    return answer;
  }
