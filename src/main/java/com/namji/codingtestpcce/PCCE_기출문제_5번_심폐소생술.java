...
public class PCCE_기출문제_5번_심폐소생술 {
  public static void main(String[] args) {
    ...
    for(int i=0; i<cpr.length; i++){ // for(int i=0; i<빈칸; i++){
      for(int j=0; j<basic_order.length; j++){ // for(int j=0; j<빈칸; j++){
        if(cpr[i].equals(basic_order[j])){
          answer[i] = j + 1; // answer[i] = 빈칸;
          break;
        }
      }
    }
