...
public class PCCE_기출문제_8번_창고_정 {
  public static void main(String[] args) {
    /*
    문자열 배열 storage와 정수 배열 num이 매개변수로 주어집니다.
    선빈이는 게임을 즐기던 중 가지고 있는 물건이 너무 많아 창고 정리를 하기로 했습니다.
    창고를 정리할 방법을 고민하던 선빈이는 같은 물건이 여러 칸에 나누어 들어있는 것을 발견
    같은 물건끼리 최대한 겹쳐쌓는 방식으로 창고를 정리하기로 했습니다.
    선빈이의 창고에 들어있는 물건의 이름과 개수는 리스트 형태로 주어지며,
    한 칸에 겹쳐질 수 있는 물건의 개수에는 제한이 없다고 가정합니다.
    예를 들어 창고의
    각 칸에 담겨있는 물건의 이름이 storage = ["pencil", "pencil", "pencil", "book"],
    ...
    for(int i=0; i<storage.length; i++){
      ...
      if(clean_idx == -1){
//        clean_storage[num_item] = Integer.parseInt(num[i]); // 수정 전
        clean_storage[num_item] = storage[i]; // 수정 후
